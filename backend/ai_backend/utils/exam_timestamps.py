"""
Exam simulation timestamps stored as readable strings for Postgres (TIMESTAMPTZ).

Format: ``YYYY-MM-DD HH:MM:SS`` (example: ``2026-03-21 11:07:18``).
Wall clock uses **Europe/Bucharest** so values match Romanian Bac context.
"""
...
def format_exam_timestamp_for_db(moment: datetime | None = None) -> str:
    ...
    Returns:
        Timestamp like ``2026-03-21 11:07:18`` (no timezone suffix; PG interprets via column type).
    """
    if moment is None:
